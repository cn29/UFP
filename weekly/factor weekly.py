...
# read data from csv
data = []
with open('EO factor contribution.csv') as csvfile:
...
for i in range(1,8):
    ...
    if 1: #= 2:
        ...
        title_str = factor_name[i+1] + ',  Mean=' + '{:.4f}'.format(a_mean) + ' ng/m$^3$'
        #ax[i - 1].title(fontdict=fontdict).set_text(factor_name[i+1])
        ax[i-1].set_title(title_str,fontdict={'fontsize': 14})
